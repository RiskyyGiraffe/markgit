            ...
            <Table>
              ...
              <TableBody>{skills.map((skill) => {
                ...
                return <TableRow key={skill.id} className="group h-[76px]">
                  ...
                  <TableCell><span className="text-sm">{skill.provenance.publisher ?? skill.provider.name}</span><span className="mt-1 block text-xs text-muted-foreground">{(skill.sourceMetadata?.popularity.stars ?? 0).toLocaleString()} source stars</span></TableCell>
