...
export default async function SkillPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const skill = await getPublicSkill(slug);
  if (!skill) notFound();
  const reviewMarkdown = await getPublicReviewMarkdown("skills", slug);
      ...
      <div className="mx-auto max-w-4xl px-5 pb-24 pt-8 sm:px-8 sm:pt-12">
        ...
        <div className="grid gap-5 pt-8 md:grid-cols-[1.4fr_0.8fr]">
          <section className="space-y-5">
            ...
            <div className="rounded-xl border bg-card p-5"><h2 className="flex items-center gap-2 text-sm font-medium"><ShieldCheck className="size-4" /> Provenance and safety</h2><dl className="mt-4 grid gap-3 text-xs sm:grid-cols-2"><div><dt className="text-muted-foreground">Publisher</dt><dd className="mt-1">{skill.provenance.publisher ?? skill.provider.name}</dd></div><div><dt className="text-muted-foreground">Revision</dt><dd className="mt-1 font-mono">{skill.provenance.revision}</dd></div><div><dt className="text-muted-foreground">Source popularity</dt><dd className="mt-1">{(skill.sourceMetadata?.popularity.stars ?? 0).toLocaleString()} GitHub stars</dd></div><div><dt className="text-muted-foreground">Snapshot digest</dt><dd className="mt-1 font-mono">{skill.sourceMetadata?.review.sha256.slice(0, 12) ?? "Not ingested"}</dd></div><div className="sm:col-span-2"><dt className="text-muted-foreground">Repository</dt><dd className="mt-1 break-all"><a href={skill.provenance.repository} className="hover:underline">{skill.provenance.repository}</a></dd></div></dl><p className="mt-4 border-t pt-4 text-xs leading-5 text-muted-foreground">Review SKILL.md, bundled scripts, references, assets, and the license before installation. Source provenance is not a security audit.</p></div>
            {reviewMarkdown ? <details className="rounded-xl border bg-card"><summary className="cursor-pointer px-5 py-4 text-sm font-medium">Review ingested SKILL.md</summary><div className="border-t p-5"><p className="mb-4 text-xs leading-5 text-muted-foreground">Exact text snapshot from the immutable source revision. Markdown is displayed as text so embedded HTML cannot execute.</p><pre className="max-h-[720px] overflow-auto whitespace-pre-wrap rounded-lg bg-muted p-4 text-xs leading-6"><code>{reviewMarkdown}</code></pre></div></details> : null}
          </section>
