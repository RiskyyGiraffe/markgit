...
const tabs: Array<{ kind?: Kind; label: string }> = [
  { label: "All" }, { kind: "tool", label: "Tools" }, { kind: "harness", label: "Custom Loops" },
  { kind: "mcp", label: "MCPs" }, { kind: "skill", label: "Skills" },
];
...
export default async function PublicSearch({ searchParams }: { searchParams: Promise<{ q?: string; kind?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#101213] text-[#f1f2f2]"><PublicHeader /><div className="mx-auto max-w-6xl px-5 pb-24 pt-12 sm:px-8">
    ...
    <nav aria-label="Registry types" className="mt-5 flex gap-1 overflow-x-auto border-b border-white/[0.09]">{tabs.map((tab) => { const params = new URLSearchParams(); if (q.trim()) params.set("q", q.trim()); if (tab.kind) params.set("kind", tab.kind); const active = tab.kind === kind || (!tab.kind && !kind); return <Link key={tab.label} href={`/search${params.size ? `?${params}` : ""}`} className={`relative whitespace-nowrap px-3 py-3 text-sm transition ${active ? "text-white after:absolute after:inset-x-2 after:bottom-0 after:h-0.5 after:rounded-full after:bg-white" : "text-[#777d81] hover:text-white"}`}>{tab.label}</Link>; })}</nav>
