            ...
            <Table>
              ...
              <TableBody>{mcps.map((mcp) => (
                <TableRow key={mcp.id} className="group h-[78px]">
                  ...
                  <TableCell><span className="text-sm">{mcp.provider.name}</span><span className="mt-1 block text-xs text-muted-foreground">{mcp.sourceMetadata ? `${mcp.sourceMetadata.popularity.stars.toLocaleString()} source stars` : mcp.provider.trustTier}</span></TableCell>
