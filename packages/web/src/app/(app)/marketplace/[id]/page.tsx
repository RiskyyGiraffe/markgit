...
export default async function ProductDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-7">
    ...
    <div className={`grid gap-6 ${product.kind === "tool" ? "lg:grid-cols-2" : ""}`}>
      ...
      <details className="group self-start rounded-xl border bg-card shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between px-6 py-5"><span><span className="block font-semibold">{product.kind === "tool" ? "Input and return schemas" : "Indexed configuration"}</span><span className="mt-1 block text-sm text-muted-foreground">Included in full-document and semantic search.</span></span><span className="text-xl text-muted-foreground transition group-open:rotate-45">+</span></summary><div className="border-t p-6"><pre className="max-h-[560px] overflow-auto rounded-lg bg-muted p-4 text-xs leading-6">{JSON.stringify({ inputSchema: product.inputSchema, outputSchema: product.outputSchema, configuration, sourceMetadata: product.sourceMetadata }, null, 2)}</pre></div></details>
    </div>
